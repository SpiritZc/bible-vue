export default {
  ...
  data() {
    return{
      pageData:{
        //请求的url start
        requestUrl:{
          listApi:"/api/tweetTopic/getTableList",//获取表格数据api
          insertApi:"/api/tweetTopic/insert",//新增用api
          updateApi:"/api/tweetTopic/update",//更新用api
          getDetailApi:"/api/tweetTopic/getDetail",//获取详情用api
          deleteOneApi:"/api/tweetTopic/delete",//单条删除api
          deleteBatchApi:"/api/tweetTopic/deletebatch",//批量删除api
        },
        //请求的url end
        //查询表单内容 start
        searchForm:[
					{type:'Input',label:'专题名',prop:'topicName'},
        ],
        //查询表单内容 end
        //查询条件 start
        queryData:{
        ...
        //查询表单按钮start
        searchHandle:[
          {label:'查询',type:'primary',handle:()=>this.searchtablelist(),auth:'tweetTopic_search'},
          {label:'重置',type:'warning',handle:()=>this.resetSearch(),auth:'tweetTopic_search'}
        ],
        //查询表单按钮end
        //表格数据start
        tableData:[],
        ...
        //表格工具栏按钮 start
        tableHandles:[
          {label:'新增',type:'primary',handle:()=>this.showModal(this.commonConstants.modalType.insert),auth:'tweetTopic_insert'},
          {label:'批量删除',type:'danger',handle:()=>this.deleteBatch(),auth:'tweetTopic_batchdelete'}
        ],
        //表格工具栏按钮 end
        selectList:[],//表格选中的数据
        //表格分页信息start
        ...
        //表格列表头start
        tableCols:[
					{label:'专题名',prop:'topicName',align:'center'},
					{label:'操作',prop:'operation',align:'center',type:'button',btnList:[
						{label:'查看',type:'text',auth:'tweetTopic_getdetail',handle:(row)=>this.showModal(this.commonConstants.modalType.detail,row.id)},
            {label:'编辑',type:'text',auth:'tweetTopic_update',handle:(row)=>this.showModal(this.commonConstants.modalType.update,row.id)},
            {label:'详情功能',type:'text',auth:'tweetTopic_datails',handle:(row)=>this.routerTo(row)},
            {label:'删除',type:'text',auth:'tweetTopic_delete',handle:(row)=>this.deleteOne(row.id)},
					]}
        ],
        //表格列表头end
        //modal配置 start
        ...
        //modal表单 start
        modalForm:[
					{type:'Input',label:'专题名',prop:'topicName',rules:{required:true,maxLength:50}},
        ],
        //modal表单 end
        //modal 数据 start
        modalData : {//modal页面数据
  ...
  methods:{
     ...
     */
    routerTo(row){
      this.$store.commit("setParameters",{key:'topicId',value:row.id});
      // this.$store.commit("setParameters",{key:'menuType',value:row.menuType});
      this.$router.push({ name: 'tweetDetails'})
    }
  }
};