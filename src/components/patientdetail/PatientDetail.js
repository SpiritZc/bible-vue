  ...
  data() {
    return{
      pageData:{
        //请求的url start
        requestUrl:{
          listApi:"/api/patientDetail/getTableList",//获取表格数据api
          // insertApi:"/api/patientDetail/insert",//新增用api
          // updateApi:"/api/patientDetail/update",//更新用api
          // getDetailApi:"/api/patientDetail//getDetail",//获取详情用api
          // deleteOneApi:"/api/patientDetail/delete",//单条删除api
          // deleteBatchApi:"/api/patientDetail/deletebatch",//批量删除api
        },
        //请求的url end
        //查询表单内容 start
        searchForm:[
					{type:'Input',label:'姓名',prop:'patientName'},
        ...
        //查询表单按钮start
        searchHandle:[
          {label:'查询',type:'primary',handle:()=>this.searchtablelist(),auth:'patientDetail_search'},
          {label:'重置',type:'warning',handle:()=>this.resetSearch(),auth:'patientDetail_search'}
        ],
        //查询表单按钮end
        //表格数据start
        tableData:[],
        ...
        //表格工具栏按钮 start
        tableHandles:[
          // {label:'新增',type:'primary',handle:()=>this.showModal(this.commonConstants.modalType.insert),auth:'patientDetail_insert'},
        ],
        //表格工具栏按钮 end
        selectList:[],//表格选中的数据
        //表格分页信息start
        ...
        //表格列表头start
        tableCols:[
					{label:'姓名',prop:'patientName',align:'center'},
					{label:'手机号',prop:'mobile',align:'center'},
          {label:'状态',prop:'status',align:'center'},
          {label:'注册时间',prop:'createTime',align:'center'},
					// {label:'操作',prop:'operation',align:'center',type:'button',btnList:[
					// 	{label:'查看',type:'text',auth:'patientDetail_getdetail',handle:(row)=>this.showModal(this.commonConstants.modalType.detail,row.id)},
					// ]}
        ],
        //表格列表头end
        //modal配置 start
        ...
        //modal表单 start
        modalForm:[
					{type:'Input',label:'',prop:'patientName',rules:{required:true,maxLength:50}},
					{type:'Input',label:'',prop:'mobile',rules:{required:true,maxLength:20,type:'mobile'}},
					{type:'Input',label:'',prop:'password',rules:{required:true,maxLength:32}},
					{type:'Input',label:'',prop:'headPic',rules:{required:true,maxLength:100}},
        ],
        //modal表单 end
        //modal 数据 start
        modalData : {//modal页面数据
