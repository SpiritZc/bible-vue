export default {
  ...
  data() {
    return{
      pageData:{
        //请求的url start
        requestUrl:{
          listApi:"/api/secondDept/getTableList",//获取表格数据api
          insertApi:"/api/secondDept/insert",//新增用api
          updateApi:"/api/secondDept/update",//更新用api
          getDetailApi:"/api/secondDept/getDetail",//获取详情用api
          deleteOneApi:"/api/secondDept/delete",//单条删除api
          deleteBatchApi:"/api/secondDept/deletebatch",//批量删除api
        },
        //请求的url end
        //查询表单内容 start
        searchForm:[
        ...
        //查询表单按钮start
        searchHandle:[
          {label:'查询',type:'primary',handle:()=>this.searchtablelist(),auth:'secondDept_search'},
          {label:'重置',type:'warning',handle:()=>this.resetSearch(),auth:'secondDept_search'}
        ],
        //查询表单按钮end
        //表格数据start
        tableData:[],
        ...
        //表格工具栏按钮 start
        tableHandles:[
          {label:'新增',type:'primary',handle:()=>this.showModal(this.commonConstants.modalType.insert),auth:'secondDept_insert'},
          {label:'批量删除',type:'danger',handle:()=>this.deleteBatch(),auth:'secondDept_batchdelete'}
        ],
        //表格工具栏按钮 end
        selectList:[],//表格选中的数据
        //表格分页信息start
        ...
        //表格列表头start
        tableCols:[
          {label:'二级科室',prop:'name',align:'center'},
          {label:'一级科室',prop:'primaryDeptName',align:'center'},
          {label:'排序',prop:'sort',align:'center'},
					{label:'操作',prop:'operation',align:'center',type:'button',btnList:[
						{label:'查看',type:'text',auth:'secondDept_getdetail',handle:(row)=>this.showModal(this.commonConstants.modalType.detail,row.id)},
						{label:'编辑',type:'text',auth:'secondDept_update',handle:(row)=>this.showModal(this.commonConstants.modalType.update,row.id)},
						{label:'删除',type:'text',auth:'secondDept_delete',handle:(row)=>this.deleteOne(row.id)},
					]}
        ],
        //表格列表头end
        //modal配置 start
  ...
  methods:{
     ...
     */    
    searchtablelist(){
      let primaryDeptId=this.$store.getters.parameters['primaryDeptId'];//获取menuId
      this.pageData.queryData.primaryDeptId = primaryDeptId;
      var obj = {
    ...
    save(){
      this.$refs['modalRef'].$refs['modalFormRef'].validate((valid) => {
        if (valid) {
            let primaryDeptId=this.$store.getters.parameters['primaryDeptId'];//获取menuId
            this.pageData.modalData.primaryDeptId = primaryDeptId;
            var obj = {
