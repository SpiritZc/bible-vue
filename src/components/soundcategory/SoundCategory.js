  ...
  data() {
    return{
      pageData:{
        ...
        //查询表单内容 start
        searchForm:[
					{type:'Input',label:'类别名',prop:'categoryName'},
        ],
        //查询表单内容 end
        //查询条件 start
        queryData:{
        ...
        //查询表单按钮start
        searchHandle:[
          {label:'查询',type:'primary',handle:()=>this.searchtablelist(),auth:'soundCategory_search'},
          {label:'重置',type:'warning',handle:()=>this.resetSearch(),auth:'soundCategory_search'}
        ],
        //查询表单按钮end
        //表格数据start
        tableData:[],
        ...
        //表格工具栏按钮 start
        tableHandles:[
          {label:'新增',type:'primary',handle:()=>this.showModal(this.commonConstants.modalType.insert),auth:'soundCategory_insert'},
          {label:'批量删除',type:'danger',handle:()=>this.deleteBatch(),auth:'soundCategory_batchdelete'}
        ],
        //表格工具栏按钮 end
        selectList:[],//表格选中的数据
        //表格分页信息start
