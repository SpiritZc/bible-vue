        ...
        <div class="el-dialog-div">
            <el-form :inline="true" ref="modalFormRef" class="demo-form-inline" :model="modalData" :disabled="modalConfig.formEditDisabled" autocomplete='modalConfig.autocomplete' :label-width="labelWidth">
                <el-form-item v-for='item in modalForm' :class="itemClass" v-show="item.show==undefined?true:item.show"  :label="item.label" :key='item.prop' :prop="item.prop" :rules="filter_rules(item.label,item.rules)" :label-width="item.labelWidth">
                    <!-- 输入框 -->
                    <el-input v-if="item.type==='Input'" :style="'width:' + item.width" v-model="modalData[item.prop]" :placeholder="'请输入'+item.label" size="item.size" :disabled="item.disabled && item.disabled(modalData[item.prop])" @change="item.change && item.change(modalData[item.prop],item.params)" @input="item.input && item.input(modalData[item.prop])"  @focus="item.focus && item.focus(modalData[item.prop])"></el-input>
                    ...
                    <!-- 单选 -->
                    <el-radio-group :style="'width:' + item.width" v-if="item.type==='Radio'" v-model="modalData[item.prop]" @change="item.change && item.change(modalData[item.prop],item.params)">
                    ...
                    <!-- textarea -->
                    <el-input v-if="item.type==='Textarea'" type="textarea" :style="'width:' + item.width" :rows="item.rows?item.rows:2" v-model="modalData[item.prop]" :placeholder="'请输入'+item.label" size="item.size" :disabled="item.disabled && item.disabled(modalData[item.prop])" @change="item.change && item.change(modalData[item.prop],item.params)" @input="item.input && item.input(modalData[item.prop])"  @focus="item.focus && item.focus(modalData[item.prop])"></el-input>
                    <!-- 省市区 -->
                    ...
                    <!-- 文件上传 -->
                    <fileUpload :style="'width:' + item.width" v-if="item.type==='Upload'" :fileList.sync="modalData[item.prop]" :accept="item.accept" :tips="item.tips" :limit="item.limit" :multiple="item.multiple" :readonly="item.readonly"></fileUpload>
                    ...
                    <div class="sub-title">{{item.suggestions}}</div>
                    ...
                    <!-- 树 -->
                     <el-tree v-if="item.type==='Tree'" 
                        ...
                        <!-- treeselect -->
                        <selectNode v-if="item.type==='TreeSelect'"  :ref="item.ref" v-model="modalData[item.prop]" :props="item.props" :options="item.data" :lazy="item.lazy" :url="item.url" :clearable="item.clearable">
                        </selectNode>
